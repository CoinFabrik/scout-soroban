...
impl TokenInterfaceEvents {
    ...
    fn decrease_balance(env: Env, to: Address, amount: i128) {
    ...
    pub fn transfer_to_admin(env: Env, from: Address, amount: i128) {
        from.require_auth();
        let balance_from = Self::balance(env.clone(), from.clone());
        assert!(amount <= balance_from);
        let admin = Self::get_metadata(env.clone()).admin;
        let admin_balance = Self::balance(env.clone(), admin.clone());
        env.storage()
            .instance()
            .set(&DataKey::Balance(admin.clone()), &(admin_balance + amount));
        TokenUtils::new(&env)
            .events()
            .transfer(from.clone(), admin.clone(), amount);
        Self::decrease_balance(env, from, amount);
    }
}
