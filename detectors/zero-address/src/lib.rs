...
fn expr_to_address_of<'hir>(kind: &'hir ExprKind<'hir>) -> Result<(&BorrowKind, &Mutability, &'hir Expr<'hir>), ()>{
...
fn path_to_type_relative<'hir>(path: &'hir QPath<'hir>) -> Result<(&'hir Ty<'hir>, &'hir PathSegment<'hir>), ()>{
...
fn match_expr_as_function_call<'hir>(expr: &'hir Expr<'hir>, type_name: &str, function_name: &str) -> Result<&'hir [Expr<'hir>], ()> {
